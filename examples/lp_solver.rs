...
fn main() {
    ...
    match solution {
        Ok(sol) => {
            print_sol(&sol);
            print_person_caledar(&sol, p, &preference);
        }
        Err(e) => {
            println!("{}", e)
        }
...
fn print_person_caledar(solution: &CoinCbcSolution, variables: Vec<Vec<Vec<Variable>>>, preferences: &[[[bool; 2]; 5]; 7]) {
